...
### Solution 1.1 - A naive approach is to take time stamps. The following is snippet is inaccurate because it relies on a single trial and because it prints the result of the subset operation to the console (which in fact takes up the bulk of the processing time).
```{r}
...
### Solution 2.2 Use {readr} with revised column specification if you need to read in data multiple times (e.g., a nightly process dumps a CSV file that needs to be processed).
```{r}
...
### Solution 2.3 - Store data.frame as an R object with `saveRDS()` for subsequent I/O and to facilitate collaboration.
```{r}
...
### Preferred Soltion #3.3 - Use package {glue}. Reads like a sentence, easy to interpret.
```{r}
library(glue)
...
### Solution 4.1 - Use nested functions
...
library(magrittr)
library(dplyr)
str(lm_fit) ## Examine the structure of this particular output
...
#### Brandon C. Loudermilk, Senior Data Scientist @ HEB (b.c.loudermilk@gmail.com)
